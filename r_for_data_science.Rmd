...
`table1`: Each row has `cases` and `population` for a single year in a single country.
`table2`: This is a long dataset (good for plotting, sometimes). Each row has some observation (either `cases` or `population`) for a single country in a single year.
`table3:` Here the numeric data (`rate`) is stored as a character vector! This is pretty obnoxious.
`table4a` & `table4b`: Each table has one row per country showing values for something with separate columns per year.
...
4. Why are `gather()` and `spread()` not perfectly symmetrical?
...
7. Why does spreading this tibble fail? How could you add a new column to fix the problem?
...
8. Tidy the simple tibble below. Do you need to spread or gather it? What are the variables?
...
9. What do the `extra` and `fill` arguments do in `separate()`? Experiment with the various options for the following two toy datasets.
...
10. Both `unite()` and `separate()` have a `remove` argument. What does it do? Why would you set it to `FALSE`?
...
11. Compare and contrast `separate()` and `extract()`. Why are there three variations of separation (by position, by separator, and with groups), but only one unite?
...
12. Compare and contrast the `fill` arguments to `spread()` and `complete()`.
...
13. What does the direction argument to `fill()` do?

It lets you choose whether to copy entries up the data frame or down the data frame.

14. In this case study I set `na.rm = TRUE` just to make it easier to check that we had the correct values. Is this reasonable? Think about how missing values are represented in this dataset. Are there implicit missing values? What’s the difference between an `NA` and zero?
...
15. What happens if you neglect the `mutate()` step? (`mutate(key = stringr::str_replace(key, "newrel", "new_rel"))`)
...
16. I claimed that `iso2` and `iso3` were redundant with country. Confirm this claim.
...
17. For each country, year, and sex compute the total number of cases of TB. Make an informative visualisation of the data.
...
5. Add a surrogate key to `flights`.
...
6.  Identify the keys in the following datasets
...
7. Draw a diagram illustrating the connections between the `Batting`, `Master`, and `Salaries` tables in the `Lahman` package. 
